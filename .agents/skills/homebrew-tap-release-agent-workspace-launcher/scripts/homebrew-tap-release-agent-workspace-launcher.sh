...
usage() {
  cat <<'USAGE'
Usage:
  homebrew-tap-release-agent-workspace-launcher.sh (--version <vX.Y.Z|X.Y.Z> | --latest) [options]

Release `agent-workspace-launcher` via the shared bump script.
...
Notes:
  ...
  - Forwarded options include:
    ...
    --no-style, --no-commit, --no-push, --no-tap-tag, --tap-tag,
    --tap-tag-prefix, --remote
USAGE
}
